...
class MyNetwork(object):
    ...
    def _build_preprocessing(self):
        ...
        with tf.variable_scope("Normalization", reuse=tf.AUTO_REUSE):
            ...
            self.n_mean = tf.get_variable(name = "n_mean_in", dtype = tf.float32,shape = (1, self.x_shp[1]))
            self.n_range = tf.get_variable(name = "n_range_in", dtype =tf.float32, shape = (1,self.x_shp[1]))
            ...
            new_n_mean = tf.assign(self.n_mean, self.n_mean_in)
            new_n_range = tf.assign(self.n_range, self.n_range_in)
            self.n_assign_op = tf.group(new_n_range, new_n_mean)
    ...
    def _build_model(self):
        ...
        with tf.variable_scope("Network", reuse=tf.AUTO_REUSE):
            ...
            cur_in = tf.layers.dense(cur_in, num_unit, kernel_initializer=kernel_initializer)
            ...
            self.logits = tf.layers.dense(cur_in, self.config.num_class, kernel_initializer=kernel_initializer)
    ...
    def _build_loss(self):
        ...
        with tf.variable_scope("Loss", reuse=tf.AUTO_REUSE):
            ...
            # logits.
            loss_i = tf.nn.softmax_cross_entropy_with_logits(labels=onehot, logits=self.logits)
    ...
    def _build_optim(self):
        ...
        with tf.variable_scope("optim", reuse=tf.AUTO_REUSE):
            ...
            self.updates = self.optim.apply_gradients(self.grads_and_vars, global_step=self.global_step)
    ...
    def _build_eval(self):
        ...
        with tf.variable_scope("Eval", tf.AUTO_REUSE):
            ...
            self.acc = tf.reduce_mean(tf.to_float(tf.equal(self.pred, self.y_in)))
    ...
    def _build_writer(self):
        ...
        self.summary_tr = tf.summary.FileWriter(self.config.log_dir + "/train")
        self.summary_va = tf.summary.FileWriter(self.config.log_dir + "/valid")
        ...
        # `self.config.log_dir`
        self.save_file_cur = tf.summary.FileWriter(self.config.log_dir, filename_suffix="model")
        # TODO: Save file for the best model. We want to do the same as above,
        # but at the `self.config.save_dir` directory
        self.save_file_best = tf.summary.FileWriter(self.config.save_dir, filename_suffix="model")
        ...
        with tf.Session() as sess:
            ...
            for idx_epoch in trange(num_epoch):
                ...
                for idx_batch in range(num_batch):
                    ...
                    self.summary_tr.add_summary(resu["self.summary_op"],global_step = idx_epoch)
                ...
                self.saver_cur.save(sess, self.save_file_cur, write_meta_graph=False, global_step=self.global_step)
